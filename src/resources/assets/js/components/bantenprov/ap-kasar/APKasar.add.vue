...
export default {
    ...
    methods: {
              ...
              if(this.model.tahun >= 2000 && this.model.tahun <= (new Date()).getFullYear())
              {
                ...
                .then(response => {
                  if(response.data.type == 'success'){
                    ...
                    window.location.href = '#/admin/ap-kasar/'+response.data.id
                  }
                  else{
                    miniToastr.error(response.data.message, response.data.title)
                    window.location.href = '#/admin/ap-kasar/create'
                  }
                },
                window.location.href = '#/admin/ap-kasar/create')
        ...
        back() {
            window.location = '#/admin/ap-kasar';
        }
    },
    mounted: function() {
        axios.get("/ap-kasar/create").then(response => {
